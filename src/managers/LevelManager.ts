...
export default class LevelManager {
    ...
    constructor(character, position, limit, elements, palette, sounds, fonts, state, gameSettings: { bestTime?: { enabled?: boolean } } = {}) {
    ...
    #resetCollectables() {
        for (const collectable of Object.values(this.collectables as Record<string, any>)) {
    ...
    #resetButterflies() {
        for (const butterfly of Object.values(this.butterflies as Record<string, any>)) {
    ...
    drawFlagpole(x) {
        ...
        this.palette.fill(
            this.flagpole.isReached ? this.palette.colors.terrain.flagpole.flag.completed : this.palette.colors.terrain.flagpole.flag.default,
        );
        ...
        triangle(
            ...
            this.flagpole.y + 3,
            poleX + poleW + 17,
            this.flagpole.y + flagH / 2,
            poleX + poleW + 3,
            this.flagpole.y + flagH - 3,
        );
    ...
    drawInstructionsText() {
        if (!this.character.hasLives()) return;
    ...
    renderClouds() {
        ...
        for (const cloud of Object.values(this.clouds as Record<string, any>)) {
    ...
    renderMountains() {
        ...
        for (const mountain of Object.values(this.mountains as Record<string, any>)) {
    ...
    renderTrees() {
        ...
        push();
        translate(this.position.cameraX * (1 - factor), 0);
        for (const tree of Object.values(this.trees as Record<string, any>)) {
            this.drawTree(tree);
        }
        pop();
    }
    ...
    checkCanyons(fnOnContact) {
        ...
        for (const canyon of this.canyons) {
            if (
                !this.character.getState("isPlummeting") &&
    ...
    checkFlagpole() {
        ...
        const isInFlagpoleAreaX =
            this.character.position.x + characterHalfW >= flagpoleArea.minX && this.character.position.x - characterHalfW <= flagpoleArea.maxX;
        ...
        if (!hasAllCollectables) return;

        const timeLeftS = this.getTimeLeftS();
        if (this.timer.enabled && timeLeftS <= 0) return;

        if (!isInFlagpoleArea) return;
    ...
    checkButterflies() {
        for (const butterfly of this.butterflies) {
            ...
            const isInContact =
                dist(butterflyPosition.x, butterflyPosition.y, this.character.position.x, this.character.position.y) <
    ...
    #createDependentPlatformLevel({ parents = [], total = 0, level = 1, widthMin = 120, widthMax = 180, minGap = 80, blockedRanges = [] }) {
        ...
        const placementRanges = Utility.normalizeRanges(
            parents.map((parent) => ({
                min: parent.x - slackX,
    ...
    #createPlatforms() {
        ...
        const levelOne = levelOneBases.map((base) =>
            this.factoryPlatform({
                x: base.x,
    ...
    #createEnemies() {
        ...
        const walkableRanges = this.#getWalkableRanges([...this.forbiddenRanges, ...protectedRanges]).filter(
            (range) => range.max - range.min >= spawn.minSegmentWidth,
        );
    ...
    #generateElements({
        ...
        minX = this.limit.gameMinXPos,
        maxX = this.limit.gameMaxXPos,
        getWidth = (scale, _index = 0) => scale,
        avoidRanges = [],
        placementRanges = undefined,
        ...
        const normalizedPlacementRanges = Utility.normalizeRanges(
            rawPlacementRanges.map((range) => ({
