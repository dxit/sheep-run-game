...
type CharacterDirectionKey = keyof CharacterDirection;
...
type CharacterStateKey = keyof CharacterState;
...
export default class CharacterManager {
    ...
    legsPosition: Record<string, string>;

    constructor(x: number, y: number, lives: number | undefined, palette: PaletteManager) {
    ...
    // Sets direction
    setDirection(direction?: CharacterDirectionKey, value?: boolean, reset = false): void {
        if (reset) {
            this.#resetDirection();
        }
    ...
    // Gets direction
    getDirection(direction: CharacterDirectionKey): boolean {
        return this.direction[direction];
    }

    // Little abstraction in order to avoid to touch the object directly and gain more control
    setState(state?: CharacterStateKey, value?: boolean, reset = false): void {
        if (reset) {
            this.#resetState();
        }
    ...
    // Gets state
    getState(state: CharacterStateKey): boolean {
        return this.state[state];
    }
    // Resets movement direction flags
    #resetDirection(): void {
        for (const key of Object.keys(this.direction) as CharacterDirectionKey[]) {
    ...
    #resetState(): void {
        for (const key of Object.keys(this.state) as CharacterStateKey[]) {
